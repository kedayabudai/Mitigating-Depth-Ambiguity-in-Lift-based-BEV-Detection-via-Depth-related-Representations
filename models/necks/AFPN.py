...
class AFPN(nn.Module):
    def __init__(self,
        ...
        if scale_factor > 1:
            self.upsampleAFPN = nn.Sequential(
                nn.Upsample(
        ...
        # 通道降维（与原AFPN保持一致的降维比例）
        self.conv0 = BasicConv(in_channels[0], in_channels[0] // 8, 1)
        self.conv1 = BasicConv(in_channels[1], in_channels[1] // 8, 1)

        # 简化版BlockBody，只处理2个输入
        self.blocks_scalezero1 = BasicConv(in_channels[0] // 8, in_channels[0] // 8, 1)
        self.blocks_scaleone1 = BasicConv(in_channels[1] // 8, in_channels[1] // 8, 1)

        # 上采样和下采样模块
        #self.downsample_scalezero1_2 = Downsample_x2(in_channels[0] // 8, in_channels[1] // 8)
        self.upsample_scaleone1_2 = Upsample(in_channels[1] // 8, in_channels[0] // 8, scale_factor=2)

        # ASFF融合模块
        self.asff_scalezero1 = ASFF_2(inter_dim=in_channels[0] // 8)
        #self.asff_scaleone1 = ASFF_2(inter_dim=in_channels[1] // 8)

        # 特征精炼模块
        self.blocks_scalezero2 = nn.Sequential(
            BasicBlock(in_channels[0] // 8, in_channels[0] // 8),
            BasicBlock(in_channels[0] // 8, in_channels[0] // 8),
        )
        #self.blocks_scaleone2 = nn.Sequential(
        #    BasicBlock(in_channels[1] // 8, in_channels[1] // 8),
        #    BasicBlock(in_channels[1] // 8, in_channels[1] // 8),
        #)
        ...
        # 额外的输出层（类似原AFPN的out4）
        #self.conv22 = nn.MaxPool2d(kernel_size=1, stride=2)
