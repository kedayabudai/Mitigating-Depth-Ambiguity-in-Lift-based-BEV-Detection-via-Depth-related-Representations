...
from mmdet3d.models.SoftDepthLoss import SoftDepthLoss
...
# 获取md4all_main的绝对路径
md4all_main_path = os.path.join(project_root, 'md4all_main')

# 确保md4all_main在Python路径中，并且放在最前面，这样它的导入优先级最高
sys.path.insert(0, md4all_main_path)
sys.path.insert(0, project_root)

# 现在可以直接导入bev_DD.py中的函数
try:
    from bev_DD import init_model, infer_depth
except ImportError:
    # 如果直接导入失败，尝试使用importlib动态导入
    bev_dd_path = os.path.join(md4all_main_path, 'bev_DD.py')
    spec = importlib.util.spec_from_file_location('bev_DD', bev_dd_path)
    bev_DD = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(bev_DD)
    init_model = bev_DD.init_model
    infer_depth = bev_DD.infer_depth
...
class BEVFusion(Base3DFusionModel):
    def __init__(
        ...
        **kwargs,
        ...
        if encoders["camera"]["DPThead"] is not None:
            self.dpt_head = init_model()
        else:
            self.dpt_head = None
        if encoders["camera"]["SAPPM"] is not None:
            self.sappm = SAPPM(in_channels=256, out_channels=256)
        else:
            self.sappm = None
    ...
    def extract_camera_features(
        ...
        if self.dpt_head is not None:
            result_depth = infer_depth(self.dpt_head, x)  #[B*N,1,H,W]
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        #show.show_bev_Averaged_Channel(x.cpu(),'fuser_bev')

        x = self.decoder["backbone"](x)#mmdet3d\models\backbones\second.py
        #x = [x[0],x[2]]
        x = self.decoder["neck"](x)#mmdet3d\models\necks\second.py
        #gate = self.decoder["neck"]._last_gate
        ...
        if self.training:
            ...
            #if beta is not None:
            #    outputs['gate_mean'] = gate.mean()
            #    outputs['gate_max'] = gate.max()
            #    outputs['beta'] = beta
