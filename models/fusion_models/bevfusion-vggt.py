...
vggt_main_path = os.path.join(project_root, 'vggt-main')

sys.path.insert(0, project_root)
sys.path.insert(0, vggt_main_path)
# 现在可以直接导入bev_DD.py中的函数

try:
    from vggt import BEVenhance
except ImportError:
    vggt_path = os.path.join(vggt_main_path, 'BEVenhance.py')
    vggt_spec = importlib.util.spec_from_file_location('BEVenhance.py', vggt_path)
    vggt = importlib.util.module_from_spec(vggt_spec)
    vggt_spec.loader.exec_module(vggt)
    BEVenhance = vggt.BEVenhance
...
class BEVFusion(Base3DFusionModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.channel_block = nn.Sequential(
            ...
            nn.BatchNorm2d(256),  # 批归一化，加速训练、稳定特征
            nn.ReLU(inplace=True)  # 非线性激活，增强表达（不改变维度）
        )
        """
        ...
        if encoders["camera"]["BEVenhance"] is not None:
            self.BEVen1 = BEVenhance(embed_dim=128,num_h=2,num_w=2,input_H=16,input_W=16)
        else:
            self.BEVen1 = None
    ...
    def extract_camera_features(
        ...
        #x = checkpoint(self.encoders["camera"]["backbone"], x)
        ...
        #x = checkpoint(self.encoders["camera"]["neck"], x)
        ...
        #show.show_feat(x.detach().cpu(),'neck')
    ...
    def forward_single(
        ...
        **kwargs,
        ...
        #show.show_bev_Averaged_Channel(x.cpu(),'fuser_bev')BEVenhance

        x = self.decoder["backbone"](x)#mmdet3d\models\backbones\second.py
        x = self.decoder["neck"](x)


        if self.BEVen1 is not None:
            y_raw = self.BEVen1(x)
            y = self.upsample(y_raw)
            x = x * (1+self.gate(y))

            
        #x = self.decoder["neck"](xe)#mmdet3d\models\necks\second.py
